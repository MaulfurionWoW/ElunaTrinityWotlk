...
*/
        ...
        void SpellHitTarget(WorldObject* target, SpellInfo const* spell) override
        {
            ...
            if (baseId == ICY_TOUCH_1 && _presence == DEATH_KNIGHT_FROST_PRESENCE && target->ToUnit()->CanHaveThreatList())
            {
                if (SpellThreatEntry const* threatEntry = sSpellMgr->GetSpellThreatEntry(spellId))
                {
                    ...
                    if (baseThreat)
                        target->ToUnit()->GetThreatManager().AddThreat(me, baseThreat * 6, spell);
                }
            }
            ...
            {
                if (Aura const* feve = target->ToUnit()->GetAura(spellId, me->GetGUID()))
                {
            ...
            {
                if (Aura* horn = target->ToUnit()->GetAura(spellId, me->GetGUID()))
                {
                ...
                if (sGameEventMgr->IsActiveEvent(GAME_EVENT_WINTER_VEIL))
                    target->ToUnit()->AddAura(44755, target->ToUnit()); //snowflakes
            }
            ...
            {
                if (Aura* fever = target->ToUnit()->GetAura(spellId, me->GetGUID()))
                {
